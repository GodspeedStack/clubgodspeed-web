...
(function () {
    ...
    /** Detect database/trigger errors that are worth retrying (transient failures) */
    ...
    /**
     * Impersonation-safe auth config.
     *
     * When a director opens a magic link generated by the admin-impersonate
     * edge function, the parent-portal URL carries `?impersonating=1`. In that
     * case we MUST NOT persist the parent session under the default storage
     * key, because parent-portal and admin-os share an origin and would
     * otherwise clobber the director's own session on localStorage.
     *
     * Strategy:
     *   - Use a namespaced storageKey: `sb-impersonation-auth-token`
     *   - Use sessionStorage so the impersonated session is scoped to this
     *     tab and auto-clears on tab close.
     *   - detectSessionInUrl stays enabled so the magic link hash is parsed.
     *
     * This isolates the director's primary admin session from the
     * short-lived parent view, eliminating a cross-tab session hijack.
     */
    function _isImpersonationContext() {
        try {
            const q = (window.location && window.location.search) || '';
            return q.indexOf('impersonating=1') !== -1;
        } catch (_) {
            return false;
        }
    }

    function _buildAuthOptions() {
        const impersonating = _isImpersonationContext();
        return {
            autoRefreshToken: true,
            persistSession: true,
            detectSessionInUrl: true,
            storage: impersonating ? window.sessionStorage : window.localStorage,
            storageKey: impersonating ? 'sb-impersonation-auth-token' : undefined,
            flowType: 'implicit'
        };
    }
        ...
        try {
            if (typeof window.supabase !== 'undefined' && window.supabase.createClient) {
                supabaseClient = window.supabase.createClient(SUPABASE_CONFIG.url, SUPABASE_CONFIG.anonKey, {
                    auth: _buildAuthOptions()
                });
                if (_isImpersonationContext()) {
                    console.info('[auth] impersonation context detected — using isolated sessionStorage key');
                }
        ...
        signup: async function (email, password, metadata = {}) {
            ...
            for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
                try {
                    ...
                    if (error) {
                        console.error(`[auth] signUp error (attempt ${attempt}/${MAX_ATTEMPTS}):`, error.message, error.status);
                        throw new Error(error.message);
                    }
                    ...
                    const isRetryable = _isDatabaseError(error) || _isNetworkError(error);

                    // Retry transient DB/network errors, but not on last attempt
                    if (isRetryable && attempt < MAX_ATTEMPTS) {
                        const backoffMs = 1500 * attempt;
                        console.warn(`[auth] signup attempt ${attempt} failed (retryable), waiting ${backoffMs}ms:`, error.message);
                        await new Promise(r => setTimeout(r, backoffMs));
                        continue;
                    }

                    // Final attempt or non-retryable error — propagate
