...
(function () {
    ...
    async function handleCheckout(button) {
        ...
        // If we're on the checkout page, proceed to Stripe
        if (window.location.pathname.includes('checkout.html')) {
            // Stripe flow is handled by checkout.html's form
            return;
        }

        // Otherwise, save cart to localStorage and redirect to checkout page
        const items = cartManager.getItems();
        localStorage.setItem('godspeed_last_order', JSON.stringify({
            items: items
        }));
