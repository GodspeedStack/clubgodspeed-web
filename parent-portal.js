...
async function handleLogin() {
 ...
 <input type="text" id="two-factor-code" placeholder="000000" maxlength="6" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600"
 pattern="[0-9]{6}">
...
function injectTrainingEvents() {
 ...
 { type: 'training', title: 'Elite Guard Academy', time: '18:00', date: '2026-01-05', loc: 'Main Court', desc: 'Pick & Roll Reads', pillClass: 'event-training',
 color: '#dcfce7',
 textColor: 'dark'
 }
 */
...
function renderParentTrips() {
 ...
 <a href="${safePaymentLink}" target="_blank" class="btn-primary" style="display:block; text-align:center; text-decoration:none; background:#0071e3; color:white; padding:12px; border-radius:8px; width:100%; font-weight:600;">
 Pay Tuition ($${safeFee})
 </a>
 </div>
 ` : ''}
...
function initPortalNav() {
 ...
 // Note: V3 uses .menu-toggle with calls to toggleSidebar() inline, // but if we want JS listener:
 if (mobileMenuBtn) {
...
async function renderTrainingDashboard() {
 ...
 <span class="badge ${isActive ? 'badge-active' : 'badge-inactive'}" style="font-size:10px; padding:2px 8px; border-radius:10px; background:${isActive ? '#dcfce7' : '#f3f4f6'}; color:${isActive ? '#166534' : '#6b7280'}; text-transform:uppercase; font-weight:700;">
 ${safeStatus}
 </span>
 </div>
