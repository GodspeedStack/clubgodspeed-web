...
CREATE POLICY "Coaches full access to teams" ON public.teams FOR ALL
    USING (EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role IN ('coach','director')));
...
CREATE POLICY "Parents upload own athlete data" ON public.data_uploads FOR INSERT
    WITH CHECK (
        ...
        OR EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role IN ('coach','director'))
    );
